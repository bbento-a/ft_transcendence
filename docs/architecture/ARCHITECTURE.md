...
│   ├── postgres_password.txt   generated by `make secrets`
│   ├── jwt_secret.txt          generated by `make secrets`
│   ├── google_client_secret.txt  filled in by hand
│   └── ft_client_secret.txt      filled in by hand
...
`server.js` plus a minimal traced `node_modules`. Receives exactly one secret,
`jwt_secret`, to verify the session cookie — no database access, no OAuth
credentials.
...
Two mechanisms, split by whether a value is a credential.

| | `.env` | `secrets/` |
|---|---|---|
| Holds | configuration: user names, host names, OAuth client ids, callback URLs | credentials: passwords, signing keys, OAuth client secrets |
| Shape | `KEY=value` lines | one bare value per file |
| Reaches the container as | `environment:` interpolation in `docker-compose.yml` | a read-only file at `/run/secrets/<name>` |
| Visible in `docker inspect` / `docker compose config` | yes | **no** |
| Created by | `make setup` (copy of `.env.example`) | `make secrets` |
| Committed | no (`.env.example` is) | no (`secrets/README.md` is) |

### Who receives what

| Secret | db | backend | frontend | nginx |
|---|:---:|:---:|:---:|:---:|
| `postgres_password` | ● | ● | | |
| `jwt_secret` | | ● | ● | |
...
Each service is listed in `docker-compose.yml` only for the secrets it needs,
and Compose mounts nothing else. nginx receives none at all.

### How a file becomes an environment variable

Postgres reads `POSTGRES_PASSWORD_FILE` natively — every `POSTGRES_*` variable
has a `_FILE` twin for exactly this.

The Node services do it in their entrypoint: it reads `/run/secrets/*` and
`exec`s the app with the values in its environment, so `ConfigService` and
`process.env` keep working and **no application code changed**. The backend
also assembles `DATABASE_URL` there, since the password now arrives as a file.

The gain over `environment:` is that the values exist only inside the process
that needs them — not in `docker compose config`, not in `docker inspect
backend`, not in a `.env` that every service could read, and not in the
scrollback of whoever ran the command.

### Rotation

`make secrets` generates `postgres_password` and `jwt_secret` from
`openssl rand -hex 32` on first run and never touches an existing file, so
re-running it is safe. The two OAuth secrets start as the placeholder
`CHANGE_ME` and are pasted in by hand; `make secrets` warns while a placeholder
is still there.

A missing or empty secret file stops the container at boot with a named error
rather than letting it start with an empty signing key.
...
| `password authentication failed` after changing `secrets/postgres_password.txt` | Postgres bakes credentials into the volume on **first** boot only | `make fclean && make up` |
| Backend restart-loops printing `!! empty secret:` or `!! missing secret:` | a file under `secrets/` is empty or gone | `make secrets` |
| `bind source path does not exist: .../secrets/*.txt` | ran `docker compose` directly on a fresh clone | `make secrets` first, or just use `make` |
| `docker compose exec backend` has no `DATABASE_URL` | `exec` starts from the *image's* environment, not the running process's | `make shell-backend`, which sources `load-secrets.sh` |
